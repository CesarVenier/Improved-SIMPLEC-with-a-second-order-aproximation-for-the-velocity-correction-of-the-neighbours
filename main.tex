...
The coupling between pressure and velocity is one of the main issues to address in Computational Fluid Dynamics (CFD) and has been a subject of discussion over the years. There are several strategies proposed in the literature which can be broadly classified as coupled or segregated methods. The first one consists on solving one system of equations which includes all the governing equations at once for all the unknowns. Examples of this approach may be found in the literature \cite{mazhar1, mazhar2, chen, darwish1, darwish2, uroic}. In these works, the pressure and velocity are assembled together in a unique system of equations where the non-linearity of the convective term is treated iteratively. Segregated algorithms, in contrast, solves the equations in a sequential fashion for a unique variable at a time. The procedure is iterative and ends when the unknowns satisfy a certain convergence criteria. In general, segregated strategies are superior in terms of memory saving and computational performance but their numerical stability may not be guaranteed for solving strongly-coupled physical problems. Within these group of methods, the penalty method \cite{temam1968methode}, the artificial compression method \cite{harten1977artificial} and projection methods \cite{chorin1, chorin2} are among the most adopted in CFD codes nowadays \cite{wang2}.
In the projection methods, an initial estimation of the velocity is corrected by means of a pressure equation where the velocity field is forced to satisfy the divergence free condition. On this line, the Fractional Step methods (FS) (based on the pioneering works of Harlow and Welch \cite{harlow} and Temam \cite{temam1968methode}) are widely-adopted in the context of the Finite Volume Method (FVM) and the Finite Elements Method (FEM). This type of techniques rely on a temporal splitting on the velocity where a first part is computed using the momentum equation with an initial guess of the pressure. Afterwards, the remaining velocity is defined by solving a pressure equation based on the mass balance.

Under the scope of the projection methods, a new line of strategies was introduced by Patankar and Spalding with the development of the SIMPLE algorithm. In this method, a pressure equation is derived after introducing the whole momentum equation into the mass balance. After the first appearance, further improvements were developed. Within the most important variants of SIMPLE may be included the SIMPLER algorithm which enhance the velocity handling of the method by adding an additional pressure equation. Another significant derivation of SIMPLE was proposed by Raithby and van Doormal who named it the SIMPLEC algorithm. It is based on including an approximation of the neighbour velocity corrections. For transient flow problems, Issa improved the SIMPLE algorithm by the inclusion of an iterative process to accurately estimate the neighbour velocity corrections. This method, called PISO, works under the hypothesis of relative low CFL numbers. Many others variants of the SIMPLE method were developed over the years. They seek to enhance the robustness and convergence rate of the simulations compared with the standard SIMPLE formulation.  A better understanding on the differences between SIMPLE-family algorithms may be found in the literature.{\color{red}Moukaled dawish, liu, miller, building}. Despite the aforementioned efforts to improve SIMPLE assumptions, this topic is a relevant and active  research field nowadays with the aim of reduce computational time and the numerical stability of the simulations {\color{red} CITAS RECIENTES (las de la revista, TAO, etc)}.


The error in the SIMPLE algorithm is exclusively  associated with the elimination of the neighbour velocity correction in the pressure equation. Without this strong assumption, the pressure equation would be exact for the linearised Navier-Stokes problem. In practice, this approximation leads to the need of relaxing the pressure to achieve a stable behaviour towards convergence in the iterative process. Regarding this, the SIMPLEC algorithm avoids relaxing the pressure by considering the neighbour velocity correction equal to the one of the current cell. In other words, the proposal is to assume the velocity correction as a constant field in the proximities of each cell. The SIMPLEC approximation may be seen as an estimation of the neighbour velocities via a zero-order truncation of a Taylor series expansion. In this context, this work proposes to employ this numerical insight by taking into account a first-order term of the same Taylor series expansion. This approach bring out the need of determining the derivative of the velocity correction which will be discussed further on. This article continues as follows: section 2 describes the SIMPLE and SIMPLEC algorithms, which are the starting point
for the present method. Then, in section 3, the basis and theoretical background of the new presented method are explained. Subsequently, in section 4, a stability analysis based on Fourier decomposition between the three algorithms is presented. Next, a series of laminar flow problems are solved to calibrate and investigate the performance of the new proposal. Finally, a discuss of the results and final comments are presented in the last section.
...
This paper presents an new strategy to couple velocity and pressure in the context of the FVM, called COMPLEX method. This proposal is based on the SIMPLEC algorithm where the neighbour velocity approximation is enhanced using a Taylor expansion. The resulting expression requires the definition of a velocity gradient which is proposed to be a scalar matrix where its value is computed solving the mass balance equation. The differences between COMPLEX and SIMPLEC may be grouped in a term, named complex term, which is relaxed in order to study its influence on the convergence of the numerical method. 
