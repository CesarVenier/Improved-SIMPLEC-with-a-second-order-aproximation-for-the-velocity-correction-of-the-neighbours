...
\end{figure}
In this problem, a constant and normal velocity of 1 m/s of magnitude is set up at the inlet of the domain. The transversal area change at the step point induces flow separation where the flow profile is adapted to the bigger section area. Analogously to the cavity case, the problem is simulated with two different viscosities, $\nu = 5.141\,e-3$ m$^2$/s and $\nu = 5.141$ m$^2$/s which refers to the Reynolds number of 389 and $0.389$ respectively. The BFS is discretized with three different mesh sizes which are defined in Table~\ref{Table:BFSMeshes}.
...
\hline 
\end{tabular}
\caption{Different mesh sizes for the BFS case.}
