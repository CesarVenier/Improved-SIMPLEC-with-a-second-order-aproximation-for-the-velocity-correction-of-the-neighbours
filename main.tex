...
quirements %[13]
, rendering it computationally more efficient %[14]
for analyzing incompressible fluid %[15]
, which is the case in built-en-
...
or segregated. The first one consists on solving one system of equations which includes all the governing equations at once for all the unknowns. Examples of this approach may be found in the literature {\color{red} CITAS Mazhar x2, Chen, Darwish x2, Uoric(Jasak)}. In these works the pressure and velocity are assembled together in a unique system of equations where the non-linearity of the convective term is treated iteratively. Segregated algorithms, in contrast, solves separately for a unique variable at a time. The procedure is iterative and ends when the unknowns satisfy a convergence criteria. Within these group of methods, the penalty method \cite{temam1968methode}, the artificial compression method \cite{harten1977artificial} and pressure correction and projection strategies \cite{chorin1,patankar1972, issa,vanDoormal,tao2001,jasak} are among the most adopted.
In the pressure correction/projection methods, an initial estimation of the velocity is corrected by means of a pressure equation that results from applying the divergence free condition of the velocity into the momentum equation.
In general, this equation is a Poisson problem that has the preliminary 
velocity divergence as independent term.
%The main difference between the pressure equation of each method is the diffusion coefficients, that result from full-coupled system approximations. In projection methods \cite{chorin1,popinet2003gerris}, for example, pressure is computed as the potential component on the Helmoltz decomposition of the divergence-free velocity and thus the diffusion coefficient only serves to achieve dimensional homogeneity.
...
\item In 1972, Spalding and Patankar \cite{patankar1972} proposed the SIMPLE algorithm for heat, mass and momentum transfer. With the SIMPLE
...
In terms of overcoming the first assumption, Patankar \cite{patankar1972,patankar1980}
...
values.
Further enhancements have been proposed to overcome the second assumption. In 1984, Raithby and van Doormal \cite{vanDoormal}
SIMPLEC algorithm which propose a simple approximation for the velocity correction in the neighbours cells. In 1986, Issa \cite{issa,issa2}
...
to the velocity correction is under consideration. Tao et al. \cite{tao2001,tao2004} improved the SIMPLER algorithm developing a fully implicit
...
the robustness. To overcome this issue, they developed the IDEAL \cite{sun2008efficient} algorithm which improves robustness applying several
...
meshes \cite{li2017efficient,xiao2018consistent}. A better understanding on the differences between SIMPLE-family algorithms has been accomplished by means of
Fourier analysis \cite{miller,miller1,miller2,miller3,liu}.
...
This paper presents an new strategy to couple velocity and pressure in the context of the FVM, named COMPLEX method. This proposal is based on the SIMPLEC algorithm where the neighbour velocity approximation is enhanced using a Taylor expansion. The resulting expression involves an unknown velocity gradient; if a scalar matrix gradient is proposed, its value can be computed by means of a mass balance equation. The gradient-based corrections  of COMPLEX can be isolated in a term, named complex term, which can be relaxed in order to study how it affects the convergence of the original SIMPLEC algorithm. 

The COMPLEX method is formally analysed using a Fourier decomposition of the error. In particular, it is compared with the SIMPLEC method in a framework of a one-dimensional Navier-Stokes problem. It is found that COMPLEX has lower error amplification factors than SIMPLEC when employing high relaxation values of the momentum equation. On the other hand, for relative low values of the relaxation factor, COMPLEX and SIMPLEC show no major differences.

The COMPLEX formulation was applied to solve two three dimensional problems: a lid driven cubic cavity case and a backward facing step (BFS) case. For each test, two different Reynolds conditions were analysed. In a first instance, the effect of the complex factor $\kappa$ was investigated through a study involving more than 1500 simulations. For each case (defined by its momentum relaxation factor, complex factor $\kappa$ and Reynolds number) the total amount of iterations to reach a convergence criteria was measured. A value around $\kappa = 0.2$ was found to have the best stability behaviour and convergence rate chosen as the most suitable value for the method in the conditions proved. After this point, the COMPLEX method was compared with SIMPLE and SIMPLEC. Here, each problem was solved using three different mesh refinements and varying the Reynolds number as the analysis before. The results proved that the COMPLEX method has advantages in terms of the convergence speed. This improvement was recognized in the range of high values of the momentum relaxation factor. This behaviour of COMPLEX was also verified via the Fourier analysis carried out in Section~\ref{sec:fourier}. This range is longer in coarse meshes which increases the advantage of COMPLEX against the other methods. Another finding was that for high Reynolds number the COMPLEX method has a better convergence than cases with relative lower Reynolds number where the relative difference with the SIMPLE algorithm was particularly significant.
