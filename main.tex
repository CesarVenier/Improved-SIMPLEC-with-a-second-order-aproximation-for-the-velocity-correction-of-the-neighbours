...
\newcommand{\COM}[1]{{\color{green} }}
\newcommand{\HA}[1]{{\color{red} }}
\newcommand{\CIP}[1]{{\color{blue} }}
\newcommand{\CMV}[1]{{\color{orange} }}
...
%\begin{enumerate}
    ...
    In the context of Computational Fluid Dynamics (CFD) based on the Finite Volume Method (FVM), pressure-velocity coupling within Navier-Stokes equations can be solved through several strategies. Broadly, the methods can be classified as direct (monolithic) or segregated. The former consist on solving one system including all equations (mass, momentum and energy) at once, giving as result all the primitive variables considered. Segregated algorithms, in contrast, split the full-coupled system in several sub-sysytems; each of them aiming to provide an estimation of a particular field.%The coupling between variables is .
    
    In these methodologies, the variables of the problem can be stored in the same or different spatial location, which gives place to grid arrangements known as collocated or staggered  respectively. The staggered grids have the advantage of avoiding interpolations and thus prevents the appearance of spurious oscillations on the unknown fields. On the other hand, collocated grid arrangements are more suitable for unstructured meshes which are mandatory to address practical applications involving complex geometries.
    ...
    In 1972, Spalding and Patankar \cite{patankar1972} proposed SIMPLE algorithm for heat, mass and momentum transfer. With the SIMPLE algorithm, pressure and velocity are decoupled so as to be solved sequentially. There are two main assumptions in SIMPLE algorithm:
    \begin{itemize}
    ...
    \end{itemize}
    These two approximations slow the convergence to a coupled system. In terms of overcoming the first assumption, Patankar \cite{patankar1980,patankar1981} proposed the SIMPLER algorithm. This improvement consists on solving an initial pressure equation based on the previous velocity values.
    
    Further enhancements have been proposed to overcome the second assumption. In 1984, Raithby and van Doormal \cite{vanDoormal} SIMPLEC algorithm which propose a simple approximation for the velocity correction in the neighbours cells. In 1986, Issa \cite{issa} presented an operator-splitting scheme for implicitly discretized equations called PISO algorithm. Thus, the neighbors’ contribution to the velocity correction is under consideration. Tao et al. \cite{tao2001,tao2004} improved the SIMPLER algorithm developing a fully implicit alternative named CLEAR without using the pressure correction equation. The CLEAR algorithm discards the second assumption so that the convergence rate is dramatically increased. However, an unexpected oscillation in the calculation procedure deteriorates the robustness. To overcome this issue, they developed the IDEAL \cite{sun2008efficient} algorithm which improves robustness applying several corrector steps. More recently, improvements have focused on stability for problems with high Reynolds number solved on fine meshes \cite{li2017efficient,xiao2018consistent}. A better understanding on the differences between SIMPLE-family algorithms has been accomplished by means of Fourier analysis \cite{liu,miller,miller3,miller1,miller2}.
    %There are also some other methods for accelerating the SIMPLE algorithm [22–28] %% Extender.
	
	%% OJO donde lo pongo a esto, pero tiene que estar

    
    %\item {\color{red} Introduccion a COMPLEX, que motivo esta propuesta, en que se basa, problematica que viene a solucionar}

	Despite the aforementioned efforts to improve SIMPLE assumptions, there are still possibilities to enhance performance and robustness of the algorithms. In this work, an extension of the SIMPLEC correction is proposed, based on applying a second order Taylor approximation to estimate velocity correction values on neighbour cells. The construction of this approximations and its usage within the algorithm require further assumptions, discussed in detail on the next sections.


    %\item {\color{red} Descripcion de las secciones del paper.}

    This article is organized as follows: section \ref{sec:theory} describes the SIMPLE and SIMPLEC algorithms, which are the starting point to define COMPLEX, in section \ref{sec:COMPLEX}, by the assumptions employed to obtain the new approximation of neighbour correction terms. A stability analysis, based on Fourier decomposition, between the three algorithms is presented in section \ref{sec:fourier}. Then, the results of each algorithm for two well-known benchmark cases, solved on two flow regimes using three mesh-size on each, are compared in section \ref{sec:cases}.
	The conclusions on section \ref{sec:conclusions} summarizes the advantages and limitations of the proposed method, discussing further improvements that could be applied in the future.
%\end{enumerate}
...
\noindent where $\displaystyle p = \frac{\mathcal{P}}{\rho}$, $\mathcal{P}$ is the pressure field, $\boldsymbol{u} = (u,v,w)$ is the velocity field, {$\rho$ is the mass density, }$\nu$ is the kinematic viscosity and $\mathbf{\Phi}$ is a momentum source term (e.g. a body force).
...
\noindent where the subscript $P$ indicates the cell-centered value of the current cell, $N$ indicates neighbor cells values and $f$ refers to the face values. The term ${b_P}\, \boldsymbol{u}^0_P$ is the contribution of the previous time-step and is function of the temporal scheme. {The face-normal vector $\textbf{S}_{f}$ has the magnitude of the face area and points out of the cell}, $F_f$ is the { velocity} flux at the face $f$ and $a_P$ and $a_{N}$ are the diagonal and off-diagonal momentum matrix coefficients respectively. The operator $\nabla p_P$ is a Gauss-based gradient of the pressure computed using first neighbors values of that field. Isolating $\boldsymbol{u}_P$ from Eq.~(\ref{eq:umom1}),
...
The COMPLEX method is analysed in the context of incompressible, laminar and stationary Newtonian flows. The performance of the method is compared with the SIMPLE and SIMPLEC methods by evaluating the convergence rate.

\subsection{Test problems}
, two typical tests for incompressible and stationary flows are studied which are the flow inside a square cavity and the backward facing step channel.
...
\caption{Geometry of the 3D cavity problem}
\label{Fig:Cavity}
\end{figure}  
...
\caption{Geometry of the 3D backward facing step problem}
...
\caption{Relative performance depending on the COMPLEX blending factor}
\label{Fig:Geometria3}
\end{figure}
...
\caption{Iterations to achieve performance with COMPLEX ($\alpha = 0.2$) in coarse mesh problems}
...
\caption{Iterations to achieve performance with COMPLEX ($\alpha = 0.2$) in medium mesh problems}
...
\caption{Iterations to achieve performance with COMPLEX ($\alpha = 0.2$) in fine mesh problems}
