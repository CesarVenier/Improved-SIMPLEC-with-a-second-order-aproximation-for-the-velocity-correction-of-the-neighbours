...
It is important to highlight that \textcolor{blue}{these values} are only function of the velocity, pressure, and of the momentum equation coefficients which are equally assembled for the three methods studied. Thus, the computation of $\overline{R}_1$ and $\overline{R}_2$ are independent of the coupling method. {\color{blue}It is also worth to notice that each of these error norms has its own dimensions,
\begin{equation}
[\overline{R_1}]
=
\text{N}
...
\text{m}^3/\text{s},
\end{equation}
and hence represent numerical errors equivalent to forces and flow ates respectively. For both cases, the simulations are considered converged if
...
\right.
\end{equation}
These thresholds should be negligible within the context of the problems. For the cubic cavity, the magnitude of the reference values are 1 m$^3$/s and 1 N for both, the flow rate and force. In the Backward Facing Step, the flow rate and the dynamic pressure force over the inlet section are 35 m$^3$/s and $17.5$ N respectively.}
