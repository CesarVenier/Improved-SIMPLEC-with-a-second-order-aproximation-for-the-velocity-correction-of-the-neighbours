...
The stability of the method is analyzed via a Fourier decomposition of the error showing a better convergence rate than SIMPLE and SIMPLEC for high relaxation factors. \textcolor{blue}{The new method is tested in two incompressible laminar flow problems. Then, the analysis is extended to a turbulent flow case. In all cases, the conclusions of the stability analysis are verified.}  The current proposal sets a theoretical baseline for further improvements and, in view of the results, it is a promising alternative to improve SIMPLE-based algorithms.
\end{abstract}
...
\boldsymbol{I}.
\end{equation}
{\color{blue}It is worth to notice that, as the velocity field $\boldsymbol{u}$ satisfies the divergence-free condition, its gradient $\nabla \boldsymbol{u}_P$ is traceless. Hence, following Eq.~(\ref{Eq:massEqSubdivided}), this means that,
\begin{equation}
\text{tr}(\nabla \boldsymbol{u}^{*}_P)
+
\text{tr}(\nabla \boldsymbol{u}^{'}_P)
...
0.
\end{equation}
Since the velocity prediction $\boldsymbol{u}^{*}$ is generally not divergence-free, its gradient is not traceless and neither is the gradient of the velocity correction $\nabla \boldsymbol{u}^{'}$. Therefore, $\alpha_P$ will not be zero in most cases.
}
...
It is important to highlight that \textcolor{blue}{these values} are only function of the velocity, the pressure, and of the momentum equation coefficients which are equally assembled for the three methods studied. Thus, the computation of $\overline{R}_1$ and $\overline{R}_2$ are independent of the coupling method. {\color{blue}It is also worth to notice that each of these error norms has dimensions, namely,
\begin{equation}
[\overline{R_1}]
=
\text{N}
...
\text{m}^3/\text{s},
\end{equation}
and, hence, these errors represent forces and flow rates respectively. For both, the cavity and BFS cases, the simulations are considered converged if the following conditions are satisfied,
...
\right.
\end{equation}
These thresholds guarantee negligible errors within the context of the proposed problems. For the cubic cavity, the magnitudes of the reference values are 1 m$^3$/s and 1 N for both, the flow rate and force. On the other hand, in the Backward Facing Step, the flow rate and the dynamic pressure force over the inlet section are equal to 35 m$^3$/s and $17.5$ N respectively.}
...
{\color{blue}
\subsection{Turbulent flow around an airfoil}
\begin{figure}[b!]
\centering
\includegraphics[width=9cm]{fig/Turbulence/Esquema.pdf}
\caption{\textcolor{blue}{Details of the problem domain and boundary conditions. Note that the airfoil profile is scaled in respect to the square domain.}}
\label{Fig:Esquema}
\end{figure}

The proposed problem studies the flow around a two-dimensional airfoil profile which is located at the center of a square domain of side $40L/3$ being $L$ the chord of the airfoil which is equal to 35. 
The boundaries of the domain are set up with a free-stream  condition where the, velocity value is equal to $\boldsymbol{U} = (25.75,\, 3.62)$ m/s which is also the initial condition over the whole domain. The details of the domain and boundary conditions are presented in Fig.~\ref{Fig:Esquema}. 

In contrast to the previous cases, this problem aims to analyze the performance of the method in the context of spatially variable viscosity due to the application of a turbulence model.

The flow turbulence is modeled with the Spalart-Almaras method where the turbulent kinematic viscosity at boundaries is $\nu_t = 0.14$. The airfoil walls are set up with a non-slip boundary condition and using a Spalding's law to define $\nu_t$ in the airfoil adjacent cells. The numerical discretization schemes are similar to those described in Section~\ref{Section:problemDescription} but the convective term is solved now with a second-order upwind scheme and non-orthogonal corrections are employed iteratively in the pressure equation. Further details of the case configuration can be found in the OpenFOAM(R) repository \cite{airfoil2dTutorial}.
...
As observed for the laminar cases, here the COMPLEX method has a better convergence rate than the other two algorithms. Moreover, in these simulations SIMPLE, uses more than  100\% of iterations than COMPLEX and SIMPLEC. Once again, the COMPLEX method surpasses SIMPLEC for high values of the moment relaxation factor which in turn leads to faster simulations.}

\subsection{Computational Performance}
\begin{figure}[t!]
\centering
\includegraphics[width=17cm]{fig/Turbulence/Todos.pdf}
\caption{\textcolor{blue}{Comparison of the number of iterations required for convergence between COMPLEX, SIMPLEC and SIMPLE algorithms for the airfoil case.}}
\label{Fig:TTodos}
\end{figure}
...
The COMPLEX algorithm was implemented using the OpenFOAM(R) libraries for the FVM \cite{ofpg}. The current implementation is compared with the OpenFOAM(R) version of SIMPLEC. The analysis shows that each COMPLEX iteration takes 5$\%$ more time than SIMPLEC for all the studied problems. The maximum computational performance is obtained using high values of the momentum relaxation factor. In these conditions, the COMPLEX method saves between 10 and 20$\%$ of iterations which, in the end, leads to higher computational efficiency. \textcolor{blue}{To clarify this point, the Fig.~\ref{Fig:TimesBFS} shows the total simulation time consumed by each method as a function of the momentum relaxation factor for the BFS case (coarse mesh, higher Reynolds number). For the particular case showed, the COMPLEX solution is the fastest at $\omega_u = 0.89$.}

\begin{figure}[t!]
\centering
\includegraphics[width=14cm]{fig/Results/TimesBFS-coarse.pdf}
\caption{\textcolor{blue}{Total simulation time as function of the momentum relaxation factor for the SIMPLE, SIMPLEC and COMPLEX method.}}
\label{Fig:TimesBFS}
\end{figure}
