...
%U(smoothsolver reltol 0), p(GAMG, GaussSeidel reltol 1e-3), res(u)=res(p)=1e-8 (ver dims). TVD(U): SOU. Grads and ints: linear. ?
...
In light of the results obtained, the COMPLEX method shows a better convergence rate than the other two algorithms. In detail, the results are analogous to those obtained in the laminar problems. Nevertheless, a relative worst performance is shown for the SIMPLE method where this simulations uses more than a 100\% of iterations than the COMPLEX and SIMPLEC. Once again, the COMPLEX method advantages SIMPLEC when using high values of the moment relaxation factor which indeed are responsible of the faster simulations in terms of the number of iterations employed.
