...
In the context of Computational Fluid Dynamics (CFD) based on the Finite Volume Method (FVM), pressure-velocity coupling
...
or segregated. The first one consists on solving one system of equations which includes all the governing equations at once for all the unknowns. Examples of this approach may be found in the literature {\color{red} CITAS Mazhar x2, Chen, Darwish x2, Uoric(Jasak)}. In these works the pressure and velocity are assembled together in a unique system of equations where the non-linearity of the convective term is treated iteratively.  
 Segregated algorithms, in contrast, solves separately for a unique variable at a time. The procedure is iterative and ends when the unknowns satisfy a convergence criteria. In general, segregated strategies are superior in terms of memory saving and computational performance but their numerical stability may not be guaranteed for solving strongly-coupled physical problems. Within these group of methods, the penalty method \cite{temam1968methode}, the artificial compression method \cite{harten1977artificial} and pressure correction and projection strategies are among the most adopted {\color{red} CHORIN}.
In the pressure correction/projection methods, an initial estimation of the velocity is corrected by means of a pressure equation where the velocity field is forced to satisfy the divergence free condition. On this line, the Fractional Step methods (FS) are widely-adopted {\color{red} CITAS}. Here, a temporal splitting on the velocity is proposed where a first part is computed using the momentum equation with an initial guess of the pressure. Afterwards, the remaining velocity is defined by solving a pressure equation based on the mass balance.

Under the scope of the projection methods, a new strategy was introduced by Patankar and Spalding with the developing of the SIMPLE algorithm. In this technique, a pressure equation is derived after introducing the whole momentum equation into the mass balance.
After the first appearance, further improvements were developed. Within the most important variants of SIMPLE may be included the SIMPLER algorithm which enhance the velocity handling of the method by adding an additional pressure equation. Another significant derivation of SIMPLE was proposed by Raithby and van Doormal who named it the
SIMPLEC algorithm. It is based on including an approximation of the neighbour velocity corrections. For transient flow problems, Issa improved the SIMPLE algorithm by the inclusion of an iterative process to accurately estimate the neighbour velocity corrections. This method, called PISO, works under the hypothesis of relative low CFL numbers. Many others variants of the SIMPLE method were developed over the years. They seek to enhance the robustness and convergence rate of the simulations compared with the standard SIMPLE formulation. 
A better understanding on the differences between SIMPLE-family algorithms may be found in the literature.{\color{red}Moukaled dawish, liu, miller, building}. Despite the aforementioned efforts to improve SIMPLE assumptions, this topic is a relevant and active  research field nowadays with the aim of reduce computational time and the numerical stability of the simulations {\color{red} CITAS RECIENTES (las de la revista, TAO, etc)}.
...
\begin{figure}[H]
    \centering
    %\includegraphics[width=0.6\textwidth]{fig/meanAmp.pdf}
    \caption{Mean amplification factor for each relaxation of momentum $\omega_u$}
    \label{fig:1d}
\end{figure}  
