...
The details of each problem are explained followed by the definition of the convergence criterion adopted. Two different analysis over the COMPLEX method are performed in laminar flow problems: first, a series of simulations are carried out to investigate the Complex factor $\kappa$, and then, a comparison of the current proposal with SIMPLE and SIMPLEC is presented for various conditions. 
Afterward, a turbulent case is analyzed using the same methodology in order to test the method performance in a RANS simulation context. Finally, the computational performance of the methods is discussed.

\subsection{Laminar flow problems description}\label{Section:problemDescription}
...
It is also worth to notice that each of these error norms has its own dimensions,% $[R_1]=$(kg m)/s$^2$ and $[R_2]=$m$^3$/s$^2$.
\begin{equation}
[\overline{R_1}]
=
...
\frac{\text{m}^3}{\text{s}},
\end{equation}
and should be normalized, dividing their values by representative forces and flowrates respectively, to obtain a dimensionless values named $\tilde{R}_1$ and $\tilde{R}_2$ that are a measure of the relative numerical error. In this context, a simulation will be considered converged if both values, $\tilde{R}_1$ and $\tilde{R}_2$  are below a tolerance of $10^{-9}$,
...
For the cubic cavity, the magnitude of the normalization values is one. In the Backward Facing Step, the flow rate and the dynamic pressure force over the inlet section are taken as normalization values.

%% Los del cavity quedan igual, porque vale todo 1. Lo del Backward Facing Step cambiaría en 1 orden de magnitud (porque la sección de entrada tiene área h x 35h = 35 m^2). El threshold con el que resolvimos entonces sería \tilde{R}_1 < 10^-9/35.... vale la pena cambiarlo? calculamos de más.4
...
\caption{Details of the problem domain and boundary conditions. Note that the airfoil profile is scaled in respecto to the square domain.}
\label{Fig:Esquema}
\end{figure}
...
The comparison of the COMPLEX method with SIMPLE and SIMPLEC is presented in Fig.\ref{Fig:TTodos} where the total number of iterations required to reach the convergence criterion of Eqs.~(\ref{Eq:convergencecriterion}) is plotted against the relaxation factor of the momentum equation, as done in the previous laminar problems.
...
Based on these results, the strategy proposed in this work set the basis to enhance the convergence rate of SIMPLE-based algorithms. The computational implementation of the COMPLEX method is straightforward and does not introduce significant computational costs compared to SIMPLEC. 
